// src/pages/DashboardPage.jsx – Final, works for both personal and organization contexts
...
function SimpleBarChart({ data, xKey, yKey, color = "#6C5CE7" }) {
  if (!data || data.length === 0)
    return (
      <p className="text-center text-gray-500 py-4">Not enough data yet</p>
    );
...
export default function DashboardPage() {
  const { user } = useAuth();
  const { activeAccount, organizations, hasPersonalAccount } = useAccount();
  ...
  useEffect(() => {
    if (activeAccount && activeAccount !== "personal") {
      getDoc(doc(db, "organizations", activeAccount))
        .then((snap) => {
  ...
  const activeOrg =
    activeAccount !== "personal"
      ? organizations.find((o) => o.id === activeAccount)
      : null;
  const contextName =
    activeAccount === "personal"
      ? "Personal"
  ...
  const canCreate = canCreatePoll(
    user,
    activeAccount,
    activeAccount !== "personal" ? activeAccount : null,
  );
    ...
    const loadAnalytics = async () => {
      try {
        ...
        if (activeAccount === "personal") {
          ...
          pollsList = snap.docs.map((doc) => ({
            id: doc.id,
            question: doc.data().question,
            type: doc.data().type,
            endsAt: toDate(doc.data().endsAt),
            meta: doc.data().meta || {},
          }));
        } else {
          const snap = await getDocs(
        ...
        for (const poll of pollsList) {
          const analyticsData = await getPollAnalytics(
            poll.id,
            user.tier,
            user.uid,
          );
  ...
  async function loadTrendData(pollsList, analyticsMap) {
    ...
    const sortedDays = Object.entries(dailyVotes)
      ...
      .slice(-30);
    setTrendData(
      sortedDays.map(([day, votes]) => ({ day: day.slice(5), votes })),
    );
  }
  ...
  const pollsLeft =
    monthlyLimit === Infinity
      ? "∞"
      : Math.max(0, monthlyLimit - (user.pollsThisMonth || 0));
  const usagePct =
    monthlyLimit === Infinity
      ? 10
      : Math.min(100, ((user.pollsThisMonth || 0) / monthlyLimit) * 100);
  const totalVotesOnMyPolls = myPolls.reduce((s, p) => s + p.totalVotes, 0);

  const filteredPolls = myPolls.filter((p) => {
    if (filter === "all") return true;
    if (filter === "active")
      return !p.endsAt || new Date(p.endsAt) > new Date();
  ...
  const tabs = [
    {
      key: "polls",
      label: `${contextName} Polls (${myPolls.length})`,
      icon: "🗳",
    },
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-5 text-center mb-4 shadow-sm">
                <div className="w-16 h-16 rounded-full bg-gradient-to-r from-primary to-secondary mx-auto flex items-center justify-center text-white text-2xl font-bold overflow-hidden mb-3">
                  {organizationInfo.logo ? (
                    ...
                    (organizationInfo.name?.[0] || "O").toUpperCase()
                  )}
                </div>
            ...
            {activeAccount === "personal" && (
              <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
                ...
                <div className="flex justify-between text-xs text-gray-500 mb-1">
                  ...
                  <span>
                    {monthlyLimit === Infinity ? "∞" : monthlyLimit} total
                  </span>
                </div>
          ...
          <div>
            ...
            <div className="lg:hidden mb-5">
              ...
              {activeAccount === "personal" && (
                <div className="bg-white rounded-xl border border-gray-100 p-3 mb-4 shadow-sm">
                  <div className="flex justify-between text-xs text-gray-500 mb-1">
                    <span>
                      Monthly polls: {user.pollsThisMonth || 0} /{" "}
                      {monthlyLimit === Infinity ? "∞" : monthlyLimit}
                    </span>
              ...
              <div>
                ...
                {activeAccount !== "personal" && activeOrg && (
                  <p className="text-xs text-primary mt-1">
                    Currently viewing <strong>{activeOrg.name}</strong>{" "}
                    (organization)
                  </p>
                )}
              </div>
            ...
            {tab !== "analytics" && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                ...
                <StatCard
                  icon="📊"
                  value={totalVotesOnMyPolls.toLocaleString()}
                  label="Votes received"
                  sub="Across this context"
                />
                <StatCard
                  icon="✅"
                  value={votes.length}
                ...
                <StatCard
                  ...
                  sub="This month"
                  color={
                    typeof pollsLeft === "number" && pollsLeft <= 1
                      ? "#ef4444"
                      : "#6C5CE7"
                  }
                />
                ...
                {filteredPolls.length === 0 ? (
                  <div className="text-center py-16 bg-white rounded-xl border border-gray-100">
                    ...
                    <p className="font-semibold text-gray-600 mb-4">
                      No polls in this context
                    </p>
                    {canCreate && (
                      <Link
                        to="/create"
                        className="bg-gradient-to-r from-primary to-secondary text-white rounded-xl px-5 py-2 text-sm font-bold shadow"
                      >
                  ...
                  <div className="space-y-3">
                    {filteredPolls.map((poll) => {
                      ...
                      return (
                        <div
                          key={poll.id}
                          className="bg-white rounded-xl border border-gray-100 p-4 flex flex-wrap items-center gap-3 transition-shadow hover:shadow-md"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <Link
                              to={`/poll/${poll.id}`}
                              className="font-bold text-gray-800 hover:text-primary line-clamp-1"
                            >
                              {poll.question}
                            </Link>
                            <div className="flex flex-wrap gap-3 text-xs text-gray-400 mt-1">
                              ...
                              <span
                                className={`flex items-center gap-1 ${isActive ? "text-green-600" : "text-gray-400"}`}
                              >
                                <span
                                  className={`w-1.5 h-1.5 rounded-full ${isActive ? "bg-green-500" : "bg-gray-300"}`}
                                />
                          ...
                          <div className="flex gap-2 flex-shrink-0">
                            ...
                            {canDelete && (
                              <button
                                onClick={() => handleDelete(poll.id, poll)}
                                disabled={deleting === poll.id}
                                className="w-8 h-8 rounded-lg border border-red-200 flex items-center justify-center text-red-500 hover:bg-red-50"
                              >
                                {deleting === poll.id ? "⏳" : "🗑️"}
                              </button>
                            )}
              ...
              (votes.length === 0 ? (
                ...
                <div className="space-y-3">
                  {votes.slice(0, 20).map((vote) => (
                    <div
                      key={vote.id}
                      className="bg-white rounded-xl border border-gray-100 p-4 flex flex-wrap items-center justify-between gap-3"
                    >
                      ...
                      <Link
                        to={`/poll/${vote.pollId}`}
                        className="text-xs font-bold text-primary bg-primary/10 px-3 py-1.5 rounded-lg shrink-0"
                      >
                        View →
                      </Link>
                    </div>
            ...
            {tab === "analytics" && canViewAnalyticsTab && (
              <div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                  <StatCard
                    icon="🗳"
                    value={analyticsOverview.totalPolls}
                    label="Total Polls"
                  />
                  <StatCard
                    icon="📊"
                    value={analyticsOverview.totalVotes.toLocaleString()}
                    label="Total Votes"
                  />
                  <StatCard
                    icon="👁"
                    value={analyticsOverview.totalViews.toLocaleString()}
                    label="Total Views"
                  />
                  <StatCard
                    icon="📤"
                    value={analyticsOverview.totalShares.toLocaleString()}
                    label="Shares"
                  />
                </div>
                    ...
                    <tbody>
                      {filteredAnalyticsPolls.map((poll) => {
                        ...
                        const voteRate = analyticsItem?.totalViews
                          ? (
                              (analyticsItem.totalVotes /
                                analyticsItem.totalViews) *
                              100
                            ).toFixed(1)
                          : 0;
                        return (
                      ...
                      <div className="bg-white rounded-xl border border-gray-100 p-5">
                        ...
                        <div className="mb-3">
                          ...
                          <p className="text-sm text-gray-600">
                            Male: {audienceProfile.gender.male} · Female:{" "}
                            {audienceProfile.gender.female} · Other:{" "}
                            {audienceProfile.gender.other}
                          </p>
                        </div>
                        <div className="mb-3">
                        ...
                        <div>
                          ...
                          {audienceProfile.topCountries.length > 0 ? (
                            <div className="flex flex-wrap gap-3 text-sm text-gray-600">
                              {audienceProfile.topCountries.map(
                                ([code, count]) => (
                                  <span key={code}>
                                    {code}: {count}
                                  </span>
