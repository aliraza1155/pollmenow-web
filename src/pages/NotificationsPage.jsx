// src/pages/NotificationsPage.jsx – dark/light mode aware
...
import { Bell, Check, Settings, Users, Star, Trophy, MessageCircle } from 'lucide-react';
import NotificationSettings from '../components/NotificationSettings';

function getIcon(type) {
  if (type.includes('vote'))       return <Check        size={16} className="text-green-500 dark:text-green-400" />;
  if (type.includes('follower'))   return <Users        size={16} className="text-orange-500 dark:text-orange-400" />;
  if (type.includes('achievement'))return <Trophy       size={16} className="text-yellow-500 dark:text-yellow-400" />;
...
export default function NotificationsPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-[#08091a]">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900 dark:text-[#f0f0ff]">Sign in to see notifications</h2>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#08091a] py-8 px-4">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-2xl font-bold text-gray-900 dark:text-[#f0f0ff]">Notifications</h1>
                {unreadCount > 0 && (
            ...
            {notifications.length === 0 ? (
              <div className="bg-white dark:bg-[#0f1120] rounded-2xl border border-gray-100 dark:border-white/8 text-center py-12">
              ...
              <div className="space-y-2">
                {notifications.map(notif => (
                  <Link
                    ...
                    className={`block rounded-xl border p-4 transition hover:shadow-md dark:hover:shadow-black/30 ${
                      !notif.read
                        ? 'bg-primary/5 dark:bg-primary/8 border-primary/30 dark:border-primary/25'
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1">
                        <h3 className="font-semibold text-gray-900 dark:text-[#f0f0ff]">{notif.title}</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-0.5">{notif.message}</p>
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">{formatRelativeTime(notif.createdAt)}</p>
                      </div>
